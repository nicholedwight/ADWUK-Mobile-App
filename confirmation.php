<?php include('inc/header.php');?>
<input type="checkbox" id="nav-trigger" class="nav-trigger" />
<label for="nav-trigger"></label>
<div class="wrapper">
  <header class="header">
    <a href="home.php"><img src="./assets/img/adwuk-logo.png" alt="Action on Disability and Work UK" class="header-logo"></a>
  </header>
  <div class="content-banner">
    <h1>Registration Complete</h1>
    <div class="breadcrumb">
      <ul>
        <li class="previous"><a href="home.php">Home</a></li>
        <li class="previous"><a href="edit.php">Edit Profile</a></li>
        <li>Registration Complete</li>
      </ul>
    </div>
  </div>
  <div id="main" role="main" class="site-wrap">
    <h1>Your account has all been set up</h1>
    <p>You can access you account details from the <a href="edit.php">My Profile</a> area in order to:</p>
    <ul class="bullets">
      <li>Update your personal details</li>
      <li>Edit your privacy settings</li>
    </ul>
    <p>If you want to jump straight into our online community, check out the <a href="forum.php">Forums</a> to get started and find some friends.</p>
  </div>
  <!-- End of site-wrap -->
</div>
<!-- End of content wrapper -->

<?php include('inc/footer.php');?>
